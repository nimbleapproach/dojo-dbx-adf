...
if ENVIRONMENT == 'dev':
    spark.sql(f"""
              DROP VIEW IF {catalog}.{schema}.vw_dim_product_staging
              """)
...
coalesce(it.sid,'NaN') as local_product_id ,
coalesce(it.ProductType,'NaN') as product_type,
(select source_system_pk from {catalog}.{schema}.dim_source_system where source_system = 'Infinigate ERP') as source_system_fk,
--  SHA2(CONCAT_WS(' ', COALESCE(TRIM(it.no_), ''), CONCAT_WS(' ', COALESCE(TRIM(it.producttype), '')),
--  COALESCE(TRIM(
--     concat(
--       regexp_replace(it.Description, 'NaN', ''),
