...
spark.sql(f"""
CREATE VIEW IF NOT EXISTS {catalog}.{schema}.fact_ig_transaction (
with msp_usage as (
  SELECT
...
cte as (
  ...
  FROM
    ...
    INNER JOIN silver_dev.igsql03.sales_invoice_line sil ON sih.No_ = sil.DocumentNo_
    ...
    /*
    LEFT JOIN silver_dev.masterdata.resellergroups AS rg ON concat(
      RIGHT(sih.Sys_DatabaseName, 2),
      sih.`Sell-toCustomerNo_`
    ) = concat(rg.Entity, rg.ResellerID)
    AND rg.Sys_Silver_IsCurrent = true -- LEFT JOIN gold_dev.obt.datanowarr ON it.No_ = datanowarr.sku
    */
...
from
  ...
  LEFT JOIN gold_dev.obt.datanowarr ON 
  case
    when msp_usage.ItemNo_ is null then cte.SKUInternal
    else msp_usage.ItemNo_
  end = datanowarr.sku
