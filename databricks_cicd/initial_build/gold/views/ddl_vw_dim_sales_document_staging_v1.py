...
spark.sql(f"""
CREATE VIEW IF NOT EXISTS {catalog}.{schema}.vw_dim_sales_document_staging (
...
AS select distinct
  ...
  (select source_system_pk from {catalog}.{schema}.source_system where source_system = 'Infinigate ERP' and is_current = 1) as source_system_id,
