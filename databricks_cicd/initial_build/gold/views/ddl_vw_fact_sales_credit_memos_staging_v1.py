...
if ENVIRONMENT == 'dev':
    spark.sql(f"""
              DROP VIEW IF EXISTS {catalog}.{schema}.vw_fact_sales_credit_memos_staging
              """)
...
FROM silver_{ENVIRONMENT}.igsql03.sales_cr_memo_header sih
...
LEFT JOIN silver_{ENVIRONMENT}.masterdata.resellergroups AS rg ON rg.ResellerID = cu.No_
AND rg.Entity = UPPER(RIGHT(sih.Sys_DatabaseName, 2))
