...
if ENVIRONMENT == 'dev':
    spark.sql(f"""
              DROP TABLE IF EXISTS {catalog}.{schema}.dim_master_vendor
              """)

# COMMAND ----------

spark.sql(f"""
CREATE TABLE IF NOT EXISTS {catalog}.{schema}.dim_master_vendor (
  master_vendor_pk BIGINT NOT NULL GENERATED BY DEFAULT AS IDENTITY (START WITH 1 INCREMENT BY 1),
  master_vendor_code STRING NOT NULL COMMENT 'Vendor Master Code',
  master_vendor_name STRING COMMENT 'The internal name text of the vendor master',
  ...
  CONSTRAINT `master_vendor_primary_key` PRIMARY KEY (`master_vendor_pk`))
USING delta
CLUSTER BY (master_vendor_code)
TBLPROPERTIES (
...
SELECT CAST(-1 AS BIGINT) AS master_vendor_pk,
       CAST('N/A' AS STRING) AS master_vendor_code,
       CAST(NULL AS STRING) AS master_vendor_name,
