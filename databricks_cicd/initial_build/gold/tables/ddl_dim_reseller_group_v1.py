...
if ENVIRONMENT == 'dev':
    spark.sql(f"""
              DROP TABLE IF EXISTS {catalog}.{schema}.dim_reseller_group
              """)

# COMMAND ----------

spark.sql(f"""
CREATE TABLE IF NOT EXISTS {catalog}.{schema}.dim_reseller_group (
  reseller_group_pk BIGINT NOT NULL GENERATED BY DEFAULT AS IDENTITY (START WITH 1 INCREMENT BY 1),
...
FROM {catalog}.{schema}.dim_reseller_group p
WHERE NOT EXISTS ( SELECT 1 FROM {catalog}.{schema}.dim_reseller_group WHERE reseller_group_pk = -1)
""").write.mode("overwrite").option("mergeSchema", "true").saveAsTable(f"{catalog}.{schema}.dim_reseller_group")
