...
if ENVIRONMENT == 'dev':
    spark.sql(f"""
              DROP TABLE IF EXISTS {catalog}.{schema}.dim_vendor_group
              """)

# COMMAND ----------

spark.sql(f"""
CREATE TABLE IF NOT EXISTS {catalog}.{schema}.dim_vendor_group (
  vendor_group_pk BIGINT NOT NULL GENERATED BY DEFAULT AS IDENTITY (START WITH 1 INCREMENT BY 1),
  vendor_group_code STRING NOT NULL COMMENT 'Vendor Code',
  vendor_group_name_internal STRING COMMENT 'The internal name text of the vendor',
  country_code  STRING COMMENT 'The country code of the vendor',
  local_vendor_group_id STRING COMMENT 'The local ID from the source system if exists',
  ...
  CONSTRAINT `dim_vendor_group_primary_key` PRIMARY KEY (`vendor_group_pk`))
USING delta
CLUSTER BY (source_system_fk,vendor_group_code)
TBLPROPERTIES (
...
sqldf = spark.sql(f"""
SELECT CAST(-1 AS BIGINT) AS vendor_group_pk,
       CAST('N/A' AS STRING) AS vendor_group_code,
       CAST(NULL AS STRING) AS vendor_group_name_internal,
       CAST(NULL AS STRING) AS country_code,
       CAST(NULL AS STRING) AS local_vendor_group_id,
...
WHERE NOT EXISTS ( SELECT 1 FROM {catalog}.{schema}.dim_vendor_group WHERE vendor_group_pk = -1 AND source_system_fk = -1)
""").write.mode("overwrite").option("mergeSchema", "true").saveAsTable(f"{catalog}.{schema}.dim_vendor_group")
