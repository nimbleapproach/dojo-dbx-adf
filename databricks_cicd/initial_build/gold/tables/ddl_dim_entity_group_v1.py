...
if ENVIRONMENT == 'dev':
    spark.sql(f"""
              DROP TABLE IF EXISTS {catalog}.{schema}.dim_entity_group
              """)

# COMMAND ----------

spark.sql(f"""
CREATE TABLE IF NOT EXISTS {catalog}.{schema}.dim_entity_group (
...
sqldf= spark.sql(f"""
SELECT CAST(-1 AS BIGINT) AS entity_group_pk,
       CAST('N/A' AS STRING) AS entity_group_code,
...
FROM {catalog}.{schema}.dim_entity_group 
WHERE NOT EXISTS (SELECT 1 FROM {catalog}.{schema}.dim_entity_group WHERE entity_group_pk = -1)
""").write.mode("overwrite").option("mergeSchema", "true").saveAsTable(f"{catalog}.{schema}.dim_entity_group")
