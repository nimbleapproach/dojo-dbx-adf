...
spark.sql(f"""
CREATE TABLE IF NOT EXISTS {catalog}.{schema}.link_entity_to_entity_group (
  entity_to_entity_group_pk BIGINT NOT NULL GENERATED BY DEFAULT AS IDENTITY (START WITH 1 INCREMENT BY 1),
  entity_group_fk BIGINT NOT NULL COMMENT 'Entity Group PK from the Dimension table',
  entity_fk BIGINT NOT NULL COMMENT 'Entity PK from the Dimension table',
  ...
  CONSTRAINT `entity_to_entity_group_primary_key` PRIMARY KEY (`entity_to_entity_group_pk`))
USING delta
CLUSTER BY (entity_group_fk)
TBLPROPERTIES (
