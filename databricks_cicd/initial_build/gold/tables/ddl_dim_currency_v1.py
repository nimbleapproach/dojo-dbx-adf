...
if ENVIRONMENT == 'dev':
    spark.sql(f"""
              DROP TABLE IF EXISTS {catalog}.{schema}.dim_currency
              """)

# COMMAND ----------
print(catalog,schema)
spark.sql(f"""
CREATE TABLE IF NOT EXISTS {catalog}.{schema}.dim_currency (
...
sqldf= spark.sql(f"""
SELECT CAST(-1 AS BIGINT) AS currency_pk,
       CAST('N/A' AS STRING) AS currency_code,
