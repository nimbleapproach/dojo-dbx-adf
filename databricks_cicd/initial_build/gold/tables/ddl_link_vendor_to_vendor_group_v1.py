...
if ENVIRONMENT == 'dev':
    spark.sql(f"""
              DROP TABLE IF EXISTS {catalog}.{schema}.link_vendor_to_vendor_group
              """)

# COMMAND ----------

spark.sql(f"""
CREATE TABLE IF NOT EXISTS {catalog}.{schema}.link_vendor_to_vendor_group (
  vendor_to_vendor_group_pk BIGINT NOT NULL GENERATED BY DEFAULT AS IDENTITY (START WITH 1 INCREMENT BY 1),
  vendor_group_fk BIGINT NOT NULL COMMENT 'Entity Group PK from the Dimension table',
  vendor_fk BIGINT NOT NULL COMMENT 'Entity PK from the Dimension table',
  ...
  CONSTRAINT `vendor_to_vendor_group_primary_key` PRIMARY KEY (`vendor_to_vendor_group_pk`))
USING delta
CLUSTER BY (vendor_group_fk, vendor_fk)
TBLPROPERTIES (
...
sqldf= spark.sql("""
SELECT CAST(-1 AS BIGINT) AS vendor_to_vendor_group_pk,
       CAST(-1 AS BIGINT) AS vendor_group_fk,
...
""").write.mode("overwrite").option("mergeSchema", "true").saveAsTable(f"{catalog}.{schema}.link_vendor_to_vendor_group")
