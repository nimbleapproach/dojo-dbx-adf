...
x = df_arr.filter(col('is_current') != 100)
...
df_trans =df_trans.filter(date_format(col('TransactionDate'), 'yyyy-MM') == '2024-10')
df_trans.write \
  .mode("overwrite") \
  .format("delta") \
...
from pyspark.sql.functions import col, lower, upper, concat_ws

# df_trans.filter(col('VendorNameMaster')=='n-able').display()
x.filter((col('vendor_code')=='n-able') & (lower(col('product_code'))==lower(lit('NAB-NC-PROF-1M-IT')))).display()
...
display(df_obt.filter(
    ...
    (date_format(col('TransactionDate'), 'yyyy-MM') == '2024-10') &
...
comparison_df.groupBy('comparison_status') \
    .agg(count('*').alias('count')) \
    .orderBy('comparison_status') \
    .display()
...
).where(date_format(col('TransactionDate'), 'yyyy-MM') == '2024-10') 
display(df_agg)
