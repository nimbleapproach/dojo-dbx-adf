...
try:
    ...
    dbutils.widgets.text(name = "wg_tableName", defaultValue = 'dati_cambio')
    TABLE_NAME = dbutils.widgets.get("wg_tableName")
...
df_test = spark.read.table(f'silver_{ENVIRONMENT}.tag02.{TABLE_NAME}')
df_test_bronze = spark.read.table(f'bronze_{ENVIRONMENT}.tag02.{TABLE_NAME}')

# COMMAND ----------

WATERMARK_COLUMN = 'DATEUPD'
...
def column_contains_positive_value(df: pyspark.sql.DataFrame, columnName : list) -> str:

    col_list = columnName
    count_list = [df_test.filter(f"""{c} <= 0""").count() for c in col_list]
    unvalidCount = 0
    column_list = []
    ...
    if unvalidCount != 0:
        return (f'ERROR, Total {unvalidCount} invalid count in following columns {column_list}".')
...
class TestTable(unittest.TestCase):
    ...
    def test_table_valid_values(self):
        ...
        """
        result = column_contains_positive_value(df_test, ['CAMBIO_PERIODO'])
        self.assertEqual(result, 'OK')
