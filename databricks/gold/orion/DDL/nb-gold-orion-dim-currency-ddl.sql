...
CREATE OR REPLACE TABLE dim_currency 
 (
  currency_pk BIGINT NOT NULL GENERATED BY DEFAULT AS IDENTITY (START WITH 1 INCREMENT BY 1),
  currency_code STRING NOT NULL COMMENT 'Currency Code',
  ...
  CONSTRAINT `dim_currency_primary_key` PRIMARY KEY (`currency_pk`))
USING delta
...
ALTER TABLE dim_currency ADD CONSTRAINT dateWithinRange_Bronze_InsertDateTime CHECK (Sys_Gold_InsertDateTime_UTC >= '1900-01-01');
ALTER TABLE dim_currency ADD CONSTRAINT dateWithinRange_Silver_InsertDateTime CHECK (Sys_Gold_ModifedDateTime_UTC >= '1900-01-01');

-- MAGIC # dim_currency default member
-- MAGIC sqldf= spark.sql("""
-- MAGIC SELECT CAST(-1 AS BIGINT) AS currency_pk,
...
-- MAGIC """).write.mode("append").option("mergeSchema", "true").saveAsTable("dim_currency")
