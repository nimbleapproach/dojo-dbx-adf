...
-- COMMAND ----------
CREATE OR REPLACE TABLE dim_entity_group
 (
  ...
  CONSTRAINT `dim_entity_group_primary_key` PRIMARY KEY (`entity_group_pk`))
USING delta
...
ALTER TABLE dim_entity_group ADD CONSTRAINT dateWithinRange_Bronze_InsertDateTime CHECK (Sys_Gold_InsertDateTime_UTC >= '1900-01-01');
ALTER TABLE dim_entity_group ADD CONSTRAINT dateWithinRange_Silver_InsertDateTime CHECK (Sys_Gold_ModifedDateTime_UTC >= '1900-01-01');

-- MAGIC # dim_entity_group default member
-- MAGIC sqldf= spark.sql("""
-- MAGIC SELECT CAST(-1 AS BIGINT) AS entity_group_pk,
-- MAGIC        CAST('N/A' AS STRING) AS entity_group_code,
...
-- MAGIC """).write.mode("append").option("mergeSchema", "true").saveAsTable("dim_entity_group")
