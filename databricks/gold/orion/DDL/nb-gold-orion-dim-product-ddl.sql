...
ALTER TABLE dim_product ADD CONSTRAINT dateWithinRange_Bronze_InsertDateTime CHECK (Sys_Gold_InsertDateTime_UTC >= '1900-01-01');
ALTER TABLE dim_product ADD CONSTRAINT dateWithinRange_Silver_InsertDateTime CHECK (Sys_Gold_ModifedDateTime_UTC >= '1900-01-01');

-- MAGIC # dim_product default member
-- MAGIC sqldf= spark.sql("""
-- MAGIC SELECT CAST(-1 AS BIGINT) AS product_pk,
-- MAGIC        CAST('N/A' AS STRING) AS product_code,
-- MAGIC        CAST(NULL AS STRING) AS product_type,
-- MAGIC        CAST(NULL AS STRING) AS local_product_id,
-- MAGIC        CAST(-1 AS BIGINT) AS source_system_id,
-- MAGIC        CAST(NULL AS STRING) AS product_description,
...
-- MAGIC """).write.mode("append").option("mergeSchema", "true").saveAsTable("dim_product")
