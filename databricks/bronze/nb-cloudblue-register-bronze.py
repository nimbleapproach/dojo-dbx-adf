...
# MAGIC %sql
# MAGIC use schema cloudblue_oss;
...
def registerTable(tableName : str, recreate : bool = False):
    ...
    if not spark.catalog.tableExists(cleanedTableName):
        ...
        Create table {cleanedTableName}
        using parquet
        Location 'abfss://bronze@adls0ig0{ENVIRONMENT}0westeurope.dfs.core.windows.net/cloudblue/oss/{tableName}/'
        """)

# COMMAND ----------

for tableName in dbutils.fs.ls('mnt/bronze/cloudblue/oss'):
    print(f"STARTING: Registering table '{tableName.name[:-1]}' to catalog 'bronze_{ENVIRONMENT}' in schema 'cloudblue_oss'.")
    registerTable(tableName.name[:-1])
    print(f"FINISHED: Registering table '{tableName.name[:-1]}' to catalog 'bronze_{ENVIRONMENT}' in schema 'cloudblue_oss'.")
